(function(window) {
    function DomColor(img, closest)  {
        this.img = img;
        this.closestEl = closest;
        this._init();
    }

    DomColor.prototype._init = function() {
        getImage(this.img);
    };
...
function joinRGB(maxProperty, image) {
    ...
    useClosest(image, this.closestEl, maxRGB);
...
function useClosest(el, parentClass, RGB) {
    ...
    window.DomColor = DomColor;

})(window);

var colorify = new DomColor('.image', '.img_container');
